...
def _module_change(script: str) -> str:
    return {
        ...
        '03_GERA_FEATURES.py': 'CUDA controlado implementado em rolling cumulativas seguras; manter auto conservador, fallback CPU e benchmark antes/depois.',
...
def build_module_details(data: dict[str, dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    improvements = [
        ...
        {'priority': 2, 'module': '03_GERA_FEATURES.py', 'improvement': 'CUDA controlado em rolling cumulativas seguras', 'why': 'Reduz potencialmente tempo em séries muito grandes sem alterar tese econômica.', 'risk_if_ignored': 'Usar CUDA sem benchmark pode piorar tempo ou gerar divergência numérica.', 'status': 'implementado / validar em run completo'},
...
def assess() -> dict[str, Any]:
    ...
    stages = [
        ...
        ['Features','OPERACIONAL / CUDA CONTROLADO','{} séries OK; {} features; backend atual {}; benchmark CUDA speedup {}.'.format(fint(feat_s.get('series_ok')), fint(feat_s.get('feature_catalog_count')), nested(feat_run_s,'feature_compute_backend','series_backend_counts'), fdec(nested(bench,'summary','speedup_cuda_vs_cpu_total'))),'Na GTX 1660 Ti o benchmark atual não mostrou ganho líquido; não tratar CUDA como melhoria econômica automática.','Rodar etapa 3 completa e comparar telemetria antes/depois.'],
        ...
        ['CUDA','IMPLEMENTADO COM CAUTELA','PyTorch CUDA={}; CuPy CUDA={}; benchmark numérico OK={}; speedup {}.'.format(nested(pyenv,'summary','cuda_ready_for_pytorch'), nested(pyenv,'summary','cuda_ready_for_cupy'), nested(bench,'summary','numerical_comparison_ok'), fdec(nested(bench,'summary','speedup_cuda_vs_cpu_total'))),'Acelerar sem equivalência numérica ou sem ganho de tempo cria falso progresso.','Manter auto conservador e revalidar após GPU nova.'],
    ]
    roadmap = [
        ...
        [3,'Validar CUDA controlado em run completo da etapa 3','Rolling cumulativas seguras foram migradas; falta medir run completo real contra CPU e futuro hardware.','IMPLEMENTADO / VALIDAR'],
